...
Device::Device(InitGraph initialize) : context(new HDevice), videoDialog(new DeviceDialogBox), crossbarDialog(new DeviceDialogBox), audioDialog(new DeviceDialogBox)
{
...
void Device::OpenDialog(void *hwnd, DialogType type) const
{
	...
	if (type == DialogType::ConfigVideo) {
	...
	} else if (type == DialogType::ConfigCrossbar || type == DialogType::ConfigCrossbar2) {
		if (context == NULL) {
		...
		hr = context->builder->FindInterface(NULL, NULL, context->videoFilter, IID_IAMCrossbar, (void **)&ptr);
		if (FAILED(hr)) {
		...
		if (ptr != NULL && type == DialogType::ConfigCrossbar2) {
			...
			hr = context->builder->FindInterface(&LOOK_UPSTREAM_ONLY, NULL, filter, IID_IAMCrossbar, (void **)&ptr);
			if (FAILED(hr)) {
	...
	if (!ptr) {
		Warning(L"Could not find filter to open dialog type: %d with", (int)type);
		return;
	}
...
static void EnumEncodedVideo(std::vector<VideoDevice> &devices, const wchar_t *deviceName, const wchar_t *devicePath, const EncodedDevice &info)
{
	VideoDevice device;
	VideoInfo caps;
...
static void EnumExceptionVideoDevice(std::vector<VideoDevice> &devices, IBaseFilter *filter, const wchar_t *deviceName, const wchar_t *devicePath)
{
	ComPtr<IPin> pin;
...
static bool EnumVideoDevice(std::vector<VideoDevice> &devices, IBaseFilter *filter, const wchar_t *deviceName, const wchar_t *devicePath)
{
	ComPtr<IPin> pin;
	...
	if (wcsstr(deviceName, L"C875") != nullptr || wcsstr(deviceName, L"Prif Streambox") != nullptr || wcsstr(deviceName, L"C835") != nullptr) {
	...
	bool success = GetFilterPin(filter, MEDIATYPE_Video, PIN_CATEGORY_CAPTURE, PINDIR_OUTPUT, &pin);
	...
	if (!success) {
		EnumExceptionVideoDevice(devices, filter, deviceName, devicePath);
		return true;
	}
	...
	info.audioAttached = GetFilterPin(filter, MEDIATYPE_Audio, PIN_CATEGORY_CAPTURE, PINDIR_OUTPUT, &audioPin);
...
{
	devices.clear();
	return EnumDevices(CLSID_VideoInputDeviceCategory, EnumDeviceCallback(EnumVideoDevice), &devices, activate);
}

static bool EnumAudioDevice(vector<AudioDevice> &devices, IBaseFilter *filter, const wchar_t *deviceName, const wchar_t *devicePath)
{
	ComPtr<IPin> pin;
	AudioDevice info;
	if (filter) {
		bool success = GetFilterPin(filter, MEDIATYPE_Audio, PIN_CATEGORY_CAPTURE, PINDIR_OUTPUT, &pin);
		if (!success)
			return true;
...
{
	devices.clear();
	return EnumDevices(CLSID_AudioInputDeviceCategory, EnumDeviceCallback(EnumAudioDevice), &devices, activate);
}
