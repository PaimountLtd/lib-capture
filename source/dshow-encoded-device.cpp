...
static inline bool CreateFilters(IBaseFilter *filter, IBaseFilter **crossbar, IBaseFilter **encoder, IBaseFilter **demuxer)
{
	...
	hr = CoCreateInstance(CLSID_MPEG2Demultiplexer, nullptr, CLSCTX_INPROC_SERVER, IID_IBaseFilter, (void **)demuxer);
	if (FAILED(hr)) {
...
static inline bool ConnectEncodedFilters(IGraphBuilder *graph, IBaseFilter *filter, IBaseFilter *crossbar, IBaseFilter *encoder, IBaseFilter *demuxer)
{
...
bool SetRocketEnabled(IBaseFilter *encoder, bool enable)
{
	...
	HRESULT hr = propertySet->Set(RocketEncoderGUID, rocketEnableId, &rocketInstance, sizeof(rocketInstance), &rocketProperty, sizeof(rocketProperty));
